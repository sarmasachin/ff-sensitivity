...
export type ChallengeListFilter = "all" | "live" | "disabled";

type Props = {
  query: string;
  filter: ChallengeListFilter;
  onQuery: (v: string) => void;
  onFilter: (v: ChallengeListFilter) => void;
  placeholder?: string;
};

const FILTERS: {
  id: ChallengeListFilter;
  label: string;
  active: string;
}[] = [
    ...
    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      ...
      <label className="relative w-full lg:max-w-[280px]">
        ...
        <input
          ...
          className="h-9 w-full rounded-xl border-0 bg-white pr-3 pl-9 text-[13px] text-[#0f172a] outline-none ring-1 ring-[#e8eaee] placeholder:text-[#94a3b8] focus:ring-2 focus:ring-orange-500/30"
        />
      </label>
    </div>
  );
