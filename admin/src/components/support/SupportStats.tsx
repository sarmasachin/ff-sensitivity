...
export function SupportStats({
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
      <Stat label="Threads" value={total} hint="All tickets" />
      <Stat label="Open queue" value={open} hint="Needs attention" />
      <Stat label="Unread" value={unread} hint="New user messages" />
      <Stat label="Replied" value={replied} hint="Staff answered" />
      <Stat label="Closed" value={closed} hint="Resolved" />
    </div>
  );
}
