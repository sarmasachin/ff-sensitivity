...
export function StaffEmptyState({ kind, onClearFilter }: Props) {
  if (kind === "filter") {
    return (
  ...
  return (
    <div className="rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-12 text-center">
      <p className="text-[15px] font-semibold text-slate-900">No staff yet</p>
      <p className="mx-auto mt-1 max-w-sm text-[13px] text-slate-500">
        Invite an Admin, Sub-Admin, or Viewer to start assigning modules.
      </p>
    </div>
  );
}
