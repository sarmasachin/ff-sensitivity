import { SETTINGS_CAPABILITIES } from "./settings-data";

const ACCENTS = [
  "border-l-stone-500 bg-stone-50/50",
  ...
  return (
    <section className="rounded-2xl border border-[#e8eaee] bg-white p-5">
      ...
      <p className="mt-1 text-[12px] text-[#94a3b8]">
        Live OpsSettings from Nest — prefs, session policy, and step-up gates.
      </p>
      <ul className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {SETTINGS_CAPABILITIES.map((item, i) => (
          <li
            key={item.title}
