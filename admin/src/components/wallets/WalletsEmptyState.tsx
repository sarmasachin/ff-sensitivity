...
export function WalletsEmptyState({ kind, onClearFilter }: Props) {
  if (kind === "filter") {
    return (
  ...
  return (
    <div className="rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-12 text-center">
      <p className="text-[15px] font-semibold text-slate-900">No wallets yet</p>
      <p className="mx-auto mt-1 max-w-sm text-[13px] text-slate-500">
        Balances appear when devices check in and the Nest coin service
        provisions a wallet row.
      </p>
    </div>
  );
}
