...
@Injectable()
export class ChallengeModuleGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!admin.allowedModules?.includes(AdminModule.daily_challenge)) {
      throw new AppError(
        'FORBIDDEN_MODULE',
        'You do not have access to Challenge.',
        403,
      );
    }
    return true;
