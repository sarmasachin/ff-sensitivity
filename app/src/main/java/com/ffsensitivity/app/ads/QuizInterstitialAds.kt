...
/**
 * Interstitial for Daily Quiz submit.
 * Submit proceeds only after the ad is shown and dismissed (or free via remote cooldown).
 */
object QuizInterstitialAds {
    @Volatile
    ...
    private fun loadIntoCache(appCtx: Context, presentPending: Boolean) {
        ...
        InterstitialAd.load(
            appCtx,
            BuildConfig.ADMOB_INTERSTITIAL_QUIZ,
            ...
            object : InterstitialAdLoadCallback() {
                ...
                override fun onAdFailedToLoad(error: LoadAdError) {
                    cached = null
                    loading = false
                    AppLog.w("Quiz interstitial load failed: ${error.message}")
                    val pending = pendingShow
                    pendingShow = null
                    pending?.let { p ->
    ...
    fun show(
        ...
        incompleteMessage: String = "Watch the ad to submit the quiz."
    ) {
        val ready = cached
        if (ready != null) {
            cached = null
