...
/**
 * Interstitial before opening a Daily redeem scratch card.
 */
object RedeemDailyInterstitialAds {
    @Volatile
    ...
    private fun loadIntoCache(appCtx: Context, presentPending: Boolean) {
        ...
        InterstitialAd.load(
            appCtx,
            BuildConfig.ADMOB_INTERSTITIAL_REDEEM_DAILY,
            ...
            object : InterstitialAdLoadCallback() {
                ...
                override fun onAdFailedToLoad(error: LoadAdError) {
                    cached = null
                    loading = false
                    AppLog.w("Redeem Daily interstitial load failed: ${error.message}")
                    val pending = pendingShow
                    pendingShow = null
                    pending?.let { p ->
    ...
    fun show(
        ...
        incompleteMessage: String = "Watch the ad to open today’s redeem card."
    ) {
        val ready = cached
        if (ready != null) {
            cached = null
